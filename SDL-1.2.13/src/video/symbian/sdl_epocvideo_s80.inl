...
void EPOC_CalcStretchFactors(_THIS,TSize aTargetSize)
{
  ...
  xStretch = 1.5;
   ...
   for(TInt x=0;x<=aTargetSize.iWidth;x++)
	  {
		  gScaleXPos[x]=(int) (0.5+x*xStretch);
		  if(x>0)
   ...
   Private->iStretchSize = TSize(xStretch*aTargetSize.iWidth,aTargetSize.iHeight);
}

inline void EPOC_S80StretchedUpdate(_THIS, int numrects, SDL_Rect *rects)
{
 ...
 TInt targetScanlineLength = Private->EPOC_ScreenSize.iWidth;
	...
	for ( i=0; i < numrects; ++i ) {
        ...
        TInt targetStartOffset = fixedOffset + gScaleXPos[rect2.x] + i240StartTable[rect2.y] * targetScanlineLength;   
...
static void EPOC_DirectUpdate(_THIS, int numrects, SDL_Rect *rects)
{
	...
	{
		EPOC_S80StretchedUpdate(_this,numrects,rects);
		return;
	}
